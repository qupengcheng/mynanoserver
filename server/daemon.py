...
from lib.hash import hex_str_to_hash,int_to_hex
...
class LegacyRPCDaemon(Daemon):
    ...
    async def make_raw_header(self, b):
        ...
        header = pack('<L', b.get('version')) \
                 ...
                 + hex_str_to_hash(b.get('reserved')) \
                 + pack('<L', self.timestamp_safe(b['time'])) \
                 + pack('<L', int(b.get('bits'), 16)) \
                 + hex_str_to_hash(b.get('nonce'))
        _len = len(b.get('solution')) / 2
        str_len = ''
        if _len < 253:
